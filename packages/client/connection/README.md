...
Set `DSH_OIDC_ENABLED=true`, `DSH_OIDC_ISSUER`, and `DSH_OIDC_CLIENT_ID` to make unauthenticated root requests use Authorization Code + PKCE against an external OIDC identity center. The callback validates the RS256 signature, issuer, audience, nonce, subject, and expiry before minting the same local DSH cookie. `DSH_OIDC_REDIRECT_URI` defaults to `http://127.0.0.1:5173/auth/oidc/callback`; standard providers such as Keycloak can omit explicit authorization, token, and JWKS endpoints.
