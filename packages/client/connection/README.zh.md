...
若设置 `DSH_OIDC_ENABLED=true`、`DSH_OIDC_ISSUER` 与 `DSH_OIDC_CLIENT_ID`，根路径未认证请求会使用 Authorization Code + PKCE 跳转到外部 OIDC 身份中心，回调会校验 RS256 签名、issuer、audience、nonce、subject 和过期时间后，才签发同一类 DSH 本地 cookie。`DSH_OIDC_REDIRECT_URI` 默认为 `http://127.0.0.1:5173/auth/oidc/callback`；Keycloak 等标准提供方可省略授权、令牌与 JWKS 地址。
